...
function Grid(){
   ...
   function handleMouseUp (){
    ...
    useEffect(()=>{ // Update the start and finish node initially
    ...
    function animateorderofvisitedNodes(){
        let orderOfVisitedNodes, distances = null
        if(isAnimating.current)return;
        ...
        if (algorithm === 'Dijkstras'){
            ({orderOfVisitedNodes, distances} = dijkstras(cellElementsRef.current, startorFinish.startNode, startorFinish.finishNode, screenHeight, screenWidth))
        } else if (algorithm === 'A Star'){
            ({orderOfVisitedNodes, distances} = A_Star(cellElementsRef.current, startorFinish.startNode, startorFinish.finishNode, screenHeight, screenWidth))
        } else {
    ...
    function animatePath(distances){
        ...
        const interval = setInterval(() => {
            ...
            if(previousNode === startorFinish.startNode){ // Check if previous node is start node if true return and clear interval
              clearInterval(interval); 
              isAnimating.current = false
              return
            }
    ...
    return(
        <div>
            <div>
                ...
                <select className="gridButton" value={algorithm} onChange={e => setAlgorithm(e.target.value)}>
                ...
                {algorithm === 'Dijkstras' || algorithm === 'A Star' ? 'Find Path' : 'Choose an Algorithm'}
                </button>   
